...
/* ─────────────────────────────────────────────────────────────
   Split-flap-display (Solari-avgangstavle).

   Portabel versjon: identisk med videomat/src/SplitFlap.jsx, men
   lyden er løftet ut til en `onClack`-callback så komponenten ikke
   drar med seg noen avhengighet. Stilene bor i videomat-look.css
   (avsnitt 7).

   props:
     text     strengen som skal stå der (uppercases selv)
     spinning true → kortene flakser mot målet
     spinKey  bump dette tallet for å starte et nytt løp
     landed   true → oransje bloom bak tegnene (resultatet er låst)
     onSettle kalles én gang når hele raden har roet seg
     compact  mindre grad (26px mot 46px)
     delay    ms ekstra før første kort lander (for å forskyve to rader)
     onClack  kalles per takt mens noe flakser — koble på lyd her

   Timingen eies av komponenten: den som spinner setter spinning +
   spinKey, og får beskjed via onSettle. Ikke prøv å time landing
   utenfra — da glipper de to fra hverandre.
   ───────────────────────────────────────────────────────────── */
...
// del teksten i 1–2 balanserte linjer så raden ikke tvinges uleselig smal
...
export default function SplitFlapDisplay({
  ...
  // når ikke i spinn: vis målteksten skarpt (også ny landet tekst)
  useEffect(() => {
  ...
  // NB: avheng av GRIDET (det som faktisk rendres), ikke `lines` — ved et nytt
  // spinn ligger gridet ett steg bak lines, og en måling på gammelt (kortere)
  // grid gir scale=1 som aldri korrigeres → lang tekst flyter ut av displayet.
  ...
  // flutter ved hvert spinn (spinKey bumpes utenfra): ÉN rAF-driver som
  ...
  return (
    <div
      className={`flap-wrap${compact ? " flap-compact" : ""}${landed ? " landed" : ""}`}
      ref={wrapRef}
      aria-hidden="true"
    >
